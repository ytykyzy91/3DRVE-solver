...
|--------|------|------|----------|-------------------|
| **CG + ILU (默认)** | `--solver cg` | 共轭梯度 + ILU预条件 + 6方向并行 | **所有规模推荐** | **~2-3分钟** |
| SPLU | `--solver splu` | 稀疏LU分解，多RHS一次求解 | 中小规模 (≤50万自由度) | ~5-10分钟 |
| CG 无预条件 | `--solver cg --cg-preconditioner none` | 纯CG迭代 | 对比测试 | ~17分钟 |
| CG + Jacobi | `--solver cg --cg-preconditioner jacobi` | 简单对角预条件 | 稳定性优先 | - |
| spsolve | `--solver spsolve` | umfpack直接法 | 调试/极小网格 | - |

**收敛残差控制：**
```bat
--solver-rtol 1e-6    (默认: 1e-6，工程足够)
--solver-rtol 1e-8    (更严格，需要更多迭代)
```

**推荐方案：**
- **65万自由度及以下**：**CG + ILU** 速度最快，内存占用也比 SPLU 小
- **100万自由度以上**：继续用 **CG + ILU**，迭代法可扩展性比直接法好得多
- **注意**：ILU 预条件构建需要约 30 秒，但迭代次数减少 10 倍以上，总体快 5-8 倍
